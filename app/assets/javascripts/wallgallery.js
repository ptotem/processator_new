...
$special,
resizeTimeout;
var background = ["1bg.jpg","2bg.jpg","3bg.jpg","4bg.jpg","5bg.jpg","6bg.jpg"];
var item = background[Math.floor(Math.random()*background.length)];
//var bhenchod;
...
var Gallery = (function() {
    ...
    function init( settings ,no_of_walls) {

        Gallery.settings = $.extend( true, {}, defaults, settings );
        // preload images
        wall_count=no_of_walls;
        $itemsContainer.imagesLoaded( buildRoom(no_of_walls) );

    }

    function buildRoom(no_of_walls) {
        // create room with 4 walls

        Gallery.room = new Room( $items,no_of_walls );
        // now show first wall (show elements of first wall)

        Gallery.room.renderWall();
        Gallery.room.changeBack();

        initEvents();
    }

    function Room( $items,no_of_walls ) {
        ...
        //this.$el.find( 'div.gr-wall-main' ).css( 'background-color', 'red' );
        //
        this.create(no_of_walls);

    }

    Room.prototype = {

        create : function(no_of_walls) {
            ...
            var pos = 0, max = 0, prev = 0,
                wallsCount = support.transforms3d ? no_of_walls : 1;
           ...
           //alert(wall_count);
            ...
            for( var i = 0; i < wall.itemsCount; ++i ) {
                ...
                if( itemH > wallH - wallmargins ) {
                    ...
                    $item.css( 'top', ( wallmargins / 1 ));
                }
                else {
                    $item.css( 'top', ( wallH - itemH ) / 1 );
                }
        ...
        changeBack : function( dir) {

            this.$mainWall.css("background","url('/assets/"+item+"')");
        },
        changeWall : function( dir ) {
            ...
            this.$auxWall.css("background","url('/assets/"+item+"')");
            ...
            if( dir === 'next' ) {
                ...
                if( support.transforms3d ) {
                    if( this.currentItem < wallItemsCount - 1 ) {
                        ...
                        // update current wall
                        this.currentWall < wall_count-1 ? ++this.currentWall : this.currentWall = 0;
                        // update wall
                        wall = this.walls[ this.currentWall ];
                        changeWall = true;
                        // reset currentItem
                ...
                if( support.transforms3d ) {
                    if( this.currentItem > 0 ) {
                        ...
                        this.lastWall = this.currentWall;
                        // update current wall
                        this.currentWall > 0 ? --this.currentWall : this.currentWall = wall_count-1;
                        // update wall
                        wall = this.walls[ this.currentWall ];
                        changeWall = true;
                        // reset currentItem
